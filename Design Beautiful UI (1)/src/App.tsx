...
// ─── Top Nav ──────────────────────────────────────────────────────────────────
function TopNav({ onChat }: { onChat: () => void }) {
  return (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        {["Dashboard", "Reports", "Audit", "Settings"].map(item => (
          <button key={item} style={{
            padding: "7px 16px", borderRadius: 20, background: item === "Dashboard" ? "rgba(255,255,255,0.35)" : "transparent",
            border: "none", cursor: "pointer",
            color: item === "Dashboard" ? "#0f172a" : "rgba(15,23,42,0.5)",
            fontSize: 13, fontFamily: "'Inter',sans-serif", fontWeight: item === "Dashboard" ? 600 : 400,
            transition: "all 0.2s",
            backdropFilter: item === "Dashboard" ? "blur(8px)" : "none",
            boxShadow: item === "Dashboard" ? "0 1px 4px rgba(0,0,0,0.08)" : "none",
          }}
            onMouseEnter={e => { if (item !== "Dashboard") (e.currentTarget as HTMLButtonElement).style.color = "#0f172a"; }}
            onMouseLeave={e => { if (item !== "Dashboard") (e.currentTarget as HTMLButtonElement).style.color = "rgba(15,23,42,0.5)"; }}
          >{item}</button>
        ))}
...
// ─── Hero section ─────────────────────────────────────────────────────────────
function Hero({ onRun, running, onChat }: { onRun: () => void; running: boolean; onChat: () => void }) {
  return (
    <div style={{
      background: "linear-gradient(160deg, #bae6fd 0%, #93c5fd 40%, #c7d2fe 100%)",
      paddingBottom: 0,
    }}>
      <TopNav onChat={onChat} />
      ...
      <div style={{
        ...
        <div style={{ position: "relative", width: "100%", paddingBottom: `${(VH / VW) * 100}%` }}>
          <div style={{ position: "absolute", inset: 0 }}>
            ...
            {/* Floating avatar — moves to current stage */}
            <div style={{
              position: "absolute",
              left: `${(avatarNode.x / VW) * 100}%`,
              top: `${(avatarNode.y / VH) * 100}%`,
              transform: "translate(-50%, -210%)",
              transition: avatarSnap ? "none" : "left 1.3s cubic-bezier(0.4,0,0.2,1), top 1.3s cubic-bezier(0.4,0,0.2,1)",
              zIndex: 10,
              pointerEvents: "none",
            }}>
              <img
                src={`https://api.dicebear.com/9.x/adventurer/svg?seed=Finley&backgroundColor=c0aede&radius=50`}
                alt="Finley"
                className="float-anim"
                style={{
                  width: 34, height: 34, borderRadius: "50%",
                  border: "2.5px solid #fff",
                  boxShadow: "0 4px 14px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.06)",
                  display: "block", animationDuration: "2.4s",
                }}
              />
            </div>
          </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            }}>{MATCH_ROWS.length} records</span>
          </div>
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
          <div style={{ position: "relative", width: 148, height: 148, flexShrink: 0 }}>
            <ResponsiveContainer width={148} height={148}>
              <PieChart>
                <Pie data={CHART_DATA} cx="50%" cy="50%" innerRadius={44} outerRadius={66}
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
            {CHART_DATA.map((d, i) => (
              <div key={d.name} onMouseEnter={() => setActive(i)} onMouseLeave={() => setActive(null)} style={{
...
function QAChat({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const send = useCallback((text?: string) => {
    ...
    const key = lower.includes("exception") ? "exception" : lower.includes("match") ? "match" : lower.includes("tier") ? "tier" : "default";
    setTimeout(() => {
      setTyping(false);
      setMsgs(m => [...m, { role: "ai", text: CANNED[key], ts: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }]);
    }, 1100);
  }, [input]);
...
export default function App() {
  ...
  const runPipeline = useCallback(() => {
    ...
    const activateAt = [400, 1900, 3500, 5100, 6700];
    const doneAt     = [1500, 3100, 4700, 6300, 7900];
    const counts     = [undefined, undefined, 47, 9, 4];

    STAGES.forEach((_, i) => {
      setTimeout(() => setStages(prev => prev.map((s, j) => ({ ...s, active: j === i, done: j < i }))), activateAt[i]);
      setTimeout(() => {
        setStages(prev => prev.map((s, j) => ({
          ...s, active: j === i ? false : s.active, done: j <= i,
          count: j === i && counts[i] !== undefined ? counts[i] : s.count,
        })));
        if (i === STAGES.length - 1) { setRunning(false); setComplete(true); setConfetti(true); }
      }, doneAt[i]);
    });
  }, [running]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f7ff" }}>
      ...
      {/* Blue hero */}
      <Hero onRun={runPipeline} running={running} onChat={() => setChatOpen(true)} />
      <WaveDivider />

      {/* White content */}
      <main style={{ padding: "24px 40px 64px", maxWidth: 1440, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <MatchTable complete={complete} />
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 16 }}>
          <ExceptionsPanel complete={complete} />
          <CategoryChart />
        </div>
